...
// BoatSpotMedia v38.8 Cancel Upload uses Delete Batch cleanup
(function(){
  if(window.__BSM_V388_CANCEL_DELETE_BATCH__) return;
  window.__BSM_V388_CANCEL_DELETE_BATCH__ = true;
  ...
  document.addEventListener("click", function(e){
    ...
    if(!ok){
      // user selected Cancel/No, continue upload
      return false;
    }

    (async function(){
      try{
        const resp = await fetch("/cancel-upload-run-delete-batch", {method:"POST"});
        try{
          console.log("Cancel Upload cleanup:", await resp.clone().json());
        }catch(err){}
      }catch(err){

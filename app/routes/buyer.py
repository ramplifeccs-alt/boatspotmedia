...
# v49.1G safe original download route.
# Does not change buyer dashboard queries. It only makes the existing button URL work.
def _bsm_r2_public_base_v491g():
    import os
    return (
...
def _bsm_public_url_from_key_v491g(key):
    if not key:
        return None
    key = str(key).strip()
    if not key:
    ...
    if key.startswith("/media/") or key.startswith("/static/"):
        return key
    return _bsm_r2_public_base_v491g() + "/" + key.lstrip("/")

@buyer_bp.route("/download-video/<int:item_id>")
def buyer_download_video_v491g(item_id):
    """
    Existing buyer dashboard may link here:
        /download-video/<order_item_id>?delivery=original
    For original/instant, redirect to the original R2 file.
    For edited, redirect to edited_r2_key if present.
    """
    ...
    try:
        ...
        if not row:
            # Some older links may pass video id instead of item id.
            row = db.session.execute(db.text("""
        ...
        if delivery in ["edited", "edit"]:
            url = _bsm_public_url_from_key_v491g(row.get("edited_r2_key"))
            if url:
                return redirect(url)

        # Original / instant download.
        for key in ["public_url", "file_path", "r2_video_key", "internal_filename"]:
            url = _bsm_public_url_from_key_v491g(row.get(key))
            if url:
                return redirect(url)
        ...
        db.session.rollback()
        try:
            print("download-video v49.1G warning:", e)
        except Exception:
            pass

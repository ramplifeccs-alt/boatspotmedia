...
@public_bp.route("/creator/register", methods=["GET","POST"], endpoint="creator_register_v492")
@public_bp.route("/creator/apply", methods=["GET","POST"], endpoint="creator_apply_v492")
@public_bp.route("/apply-creator", methods=["GET","POST"], endpoint="apply_creator_v492")
@public_bp.route("/apply", methods=["GET","POST"], endpoint="apply_v492")
def creator_register_v492():
    if request.method == "POST":
        first_name = (request.form.get("first_name") or "").strip()
        last_name = (request.form.get("last_name") or "").strip()
        brand_name = (request.form.get("brand_name") or request.form.get("company_name") or "").strip()
        email = (request.form.get("email") or "").strip().lower()
        ...
        if not first_name or not last_name or not brand_name or not email:
            flash("Please complete first name, last name, brand/company name, and email.")
            return render_template("public/apply_creator.html", submitted=False)

        try:
            # Existing real table. Do not create duplicate tables.
            db.session.execute(db.text("ALTER TABLE creator_application ADD COLUMN IF NOT EXISTS phone TEXT"))
            ...
            db.session.execute(db.text("ALTER TABLE creator_application ADD COLUMN IF NOT EXISTS notes TEXT"))

            db.session.execute(db.text("""
                INSERT INTO creator_application
                (first_name, last_name, brand_name, email, phone, instagram, location, website, notes, status, submitted_at)
                VALUES
                (:first_name, :last_name, :brand_name, :email, :phone, :instagram, :location, :website, :notes, 'pending', CURRENT_TIMESTAMP)
            """), {
                "first_name": first_name,
                "last_name": last_name,
                "brand_name": brand_name,
            ...
            db.session.rollback()
            try: print("creator register v49.2 warning:", e)
            except Exception: pass
            flash("Could not submit application. Please try again.")
...
# v49.5 robust logout only. OAuth routes are left original.
@public_bp.route("/logout", endpoint="logout_v495")

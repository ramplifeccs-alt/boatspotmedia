...
# v49.1X creator Stripe payout requirement helpers
def _bsm_creator_has_stripe_connected_v491x(creator_id):
    try:
        acct = _bsm_creator_stripe_account_id_v491l(creator_id) if "_bsm_creator_stripe_account_id_v491l" in globals() else ""
        return bool((acct or "").strip())
    except Exception:
        return False

def _bsm_creator_require_stripe_v491x(creator_id, action_label="continue"):
    if _bsm_creator_has_stripe_connected_v491x(creator_id):
        return None
    flash("Action required: Connect Stripe to receive payouts from your video sales before you " + action_label + ".")
...
def pricing():
    # require Stripe Connect before creator action v49.1X
    c = current_creator()
    if not c:
        return redirect("/creator/login")
    _stripe_required_resp_v491x = _bsm_creator_require_stripe_v491x(c.id, "set prices")
...
def _bsm_normalize_creator_phone_v505ab(phone):
    phone = (phone or "").strip()
    if not phone:
        return ""
...
def creator_settings_v505ab():
    ...
    def _load_application():
        try:
            ...
            db.session.rollback()
            try:
                print("creator settings load creator_application v50.5AB warning:", e)
            except Exception:
                pass
            return None
    ...
    # Do not break Settings if creator_application is missing; use user row as fallback.
    if not app_row:
        try:
            app_row = db.session.execute(db.text("""
                SELECT NULL::integer AS id,
                       ...
                       COALESCE(display_name, public_name, email) AS brand_name,
                       email,
    ...
    if not app_row:
        flash("Creator profile not found for this email.")
        return redirect("/creator/dashboard")
    ...
    if request.method == "POST":
        ...
        phone = _bsm_normalize_creator_phone_v505ab(request.form.get("phone") or "")
        instagram = (request.form.get("instagram") or "").strip().replace("@", "")
        website = (request.form.get("website") or "").strip()
        location = (request.form.get("location") or "").strip()

        try:
            existing_app = db.session.execute(db.text("""
                SELECT id FROM creator_application
                WHERE lower(COALESCE(email,'')) = lower(:old_email)
                ORDER BY CASE WHEN COALESCE(status,'')='approved' THEN 0 ELSE 1 END, id DESC
                LIMIT 1
            """), {"old_email": session_email}).mappings().first()
            ...
            # Update the real login user too.
            try:
                db.session.execute(db.text("""
                    UPDATE "user"
                    SET email=:email,
                ...
                db.session.rollback()
                # retry without optional columns if older schema
                db.session.execute(db.text("""
                    UPDATE "user"
                    SET email=:email,
                        display_name=:brand_name
            ...
            # Keep creator_profile instagram in sync when possible.
            try:
                db.session.execute(db.text("""
                    UPDATE creator_profile
                    SET instagram=:instagram
                    WHERE user_id=:uid
                """), {"uid": user_id, "instagram": instagram})
            except Exception:
                db.session.rollback()
            ...
            db.session.rollback()
            try:
                print("creator settings save v50.5AB warning:", e)
            except Exception:
                pass
            flash("Could not save settings.")
            return redirect("/creator/settings")

    app_row = _load_application() or app_row
...
def _creator_used_storage_bytes(creator_id):
    from app.models import Video
    original_total = db.session.query(db.func.coalesce(db.func.sum(Video.file_size_bytes), 0)).filter(
        Video.creator_id == creator_id,
        db.or_(Video.status == None, ~Video.status.in_(["deleted", "cancelled", "canceled"]))
    ).scalar()
    edited_total = 0
    try:
        ...
        db.session.commit()
        row = db.session.execute(db.text("""
            SELECT COALESCE(SUM(COALESCE(i.edited_file_size_bytes,0)),0) AS total
            FROM bsm_cart_order_item i
            LEFT JOIN video v ON v.id = i.video_id
            WHERE (i.creator_id = :creator_id OR v.creator_id = :creator_id)
        ...
        db.session.rollback()
    return int(original_total or 0) + int(edited_total or 0)
...
def creator_support_center_v505c():
    ...
    creator_id=_bsm_creator_id_v505c()
    creator_email=session.get("user_email") or session.get("creator_email")

    if request.method=="POST":
        subject=(request.form.get("subject") or "Platform support request").strip()[:180]
        ...
        try:
            res=db.session.execute(db.text("""
                INSERT INTO support_thread(thread_type, subject, creator_id, status, last_message_at, created_at, updated_at)
                VALUES ('creator_owner', :subject, :creator_id, 'open', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                RETURNING id
            """), {"subject":subject,"creator_id":creator_id})
            tid=res.scalar()
            db.session.execute(db.text("""
                INSERT INTO support_message(thread_id, sender_role, sender_id, sender_email, body, created_at)
                VALUES (:tid, 'creator', :sid, :email, :body, CURRENT_TIMESTAMP)
    ...
    try:
        ...
        db.session.rollback(); buyer_threads=[]

    try:
        platform_threads=db.session.execute(db.text("""
            SELECT st.*,
                   (SELECT body FROM support_message sm WHERE sm.thread_id=st.id ORDER BY sm.created_at DESC, sm.id DESC LIMIT 1) AS last_body
            FROM support_thread st
            WHERE st.thread_type='creator_owner' AND st.creator_id=:creator_id
            ORDER BY st.last_message_at DESC, st.id DESC
        """), {"creator_id":creator_id}).mappings().all()
    except Exception:
    ...
    if request.method=="POST":
        ...
        if body:
            ...
            try:
                db.session.execute(db.text("""
                    INSERT INTO support_message(thread_id, sender_role, sender_id, sender_email, body, created_at)
                    VALUES (:tid, 'creator', :sid, COALESCE(:email, ''), :body, CURRENT_TIMESTAMP)
                """), {"tid":thread_id,"sid":creator_id,"email":creator_email,"body":body})
